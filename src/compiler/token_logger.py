...
def log_token_usage(model_name: str, prompt_tokens: int, candidate_tokens: int, total_tokens: int, task_name: str = "compiler"):
    ...
    with open(LOG_FILE, mode='a', newline='') as csvfile:
        fieldnames = ['timestamp', 'task_name', 'model', 'prompt_tokens', 'candidate_tokens', 'total_tokens', 'estimated_cost']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        if not file_exists:
            writer.writeheader()
        
        # Note: If file exists but header is missing 'estimated_cost', DictWriter handles it gracefully 
