 ...
 */
export function useTransformEditor(asset, refresh, executeCommand, updateAssetLocal) {
    ...
    const handleSpritePositionChanged = useCallback(async (spriteName, x, y, time) => {
        ...
        if (updateAssetLocal) {
            ...
            const layer = (updatedConfigLocal.layers || []).find(l => l.sprite_name === spriteName);
            if (layer) {
                ...
                if (existingIndex >= 0) layer.behaviors[existingIndex] = locationBehavior;
                else {
                    layer.behaviors.push(locationBehavior);
                    layer.behaviors.sort((a, b) => (a.time_offset || 0) - (b.time_offset || 0));
                }
                updateAssetLocal('scene', asset.name, updatedConfigLocal);
            }
        }
    ...
    }, [asset.config, asset.name, refresh, executeCommand, updateAssetLocal]);
        ...
        if (updateAssetLocal) {
            ...
            if (layer) {
                if (time > 0.1) {
                    ...
                    if (loc) loc.rotation = degrees;
                    else layer.rotation = degrees;
                }
                updateAssetLocal('scene', asset.name, updatedConfigLocal);
            }
        }
    ...
    const handleKeyframeMove = useCallback(async (layerName, behaviorIndex, newTime, commit) => {
        ...
        if (updateAssetLocal) {
            const updatedConfigLocal = JSON.parse(JSON.stringify(asset.config));
            const layer = (updatedConfigLocal.layers || []).find(l => l.sprite_name === layerName);
    ...
    }, [asset.config, asset.name, refresh, updateAssetLocal]);
