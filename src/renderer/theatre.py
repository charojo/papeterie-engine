...
class ParallaxLayer:
    ...
    def draw(
        ...
        if self.environmental_reaction and env_layer:
            ...
            # 4. Smoothing / Inertia
            # Rotation smoothing
            smoothing = 1.0 - math.exp(-15.0 * dt)
            self.current_tilt += (target_tilt - self.current_tilt) * smoothing
            ...
            if reaction.vertical_follow_factor > 0:
                ...
                y_smoothing = 1.0 - math.exp(-15.0 * dt)
                self._current_y_phys += (desired_y - self._current_y_phys) * y_smoothing
