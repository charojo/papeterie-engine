...
*   **[DONE-022] PythonAnywhere Deployment**: Implemented `scripts/setup_pythonAnywhere.py`, `prepare_deploy.sh`, and comprehensive documentation (`docs/HOWTO_Deploy_PythonAnywhere.md`). Design: [`docs/design/deployment.md`](design/deployment.md).
*   **[TASK-004] AWS Deployment Guide**: Create a similar HOWTO and script set for AWS (Elastic Beanstalk or EC2). Goals: Full processing capabilities (no disk quota limits), S3 integration for assets.
*   **[TASK-005] Docker Containerization**: Create a `Dockerfile` for the engine to simplify deployment to any container runner (AWS ECS, Google Cloud Run, etc.).

## Multi-Repo Architecture Roadmap

This section tracks the transition to a "Meta-Layer" architecture. Design: [`docs/design/multi_repo_architecture.md`](design/multi_repo_architecture.md).

*   **[TASK-006] Extract Agent Environment**: Publish `.agent` as `agent-dev-environment` repo.
*   **[TASK-007] Open-Source Inkado**: Audit secrets, add MIT license, make public.
*   **[TASK-008] Create Meta-Repo (papeterie-suite)**: Container repo with submodules for Engine, Inkado, and Agent.
*   **[TASK-009] Unified PythonAnywhere Deployment**: Deploy suite with WSGI dispatcher serving both apps.

## Active Development
*   **[TASK-003] CI/CD Clean Pull Validation**: Implement a GitHub Action or similar CI pipeline to automate the "Clean Pull & Build" validation (`scripts/validate.sh --full` on a fresh state). This replaces the need for the manual "nuclear" reset script and ensures every PR is buildable from scratch.
