...
pytestmark = pytest.mark.xdist_group("pygame_mocks")
...
def dummy_png(tmp_path):
    """Create a unique dummy PNG file for each test to avoid race conditions."""
...
def test_environmental_reaction_pivot_on_crest(mocker, dummy_png):
    ...
    # smoothing = 1 - exp(-15 * 0.016) approx 0.2134
    # 28.55 * 0.2134 = 6.09
    ...
    assert args[1] == pytest.approx(6.09, abs=0.1)

    # Check vertical position: target_env_y = (y_stern + y_bow) / 2 = 405
    # Step 1 (dt=0.016): starts at 335, target 400 -> result 348.87
    # Step 2 (dt=0.016): starts at 348.87, target 405 -> result 360.85
    assert boat_layer._current_y_phys == pytest.approx(360.85, abs=1.0)
...
def test_rotation_position_stability(dummy_png, mocker):
    ...
    assert mock_screen.blit.called, "blit was not called for rotation 0"
