...
def test_environmental_reaction_pivot_on_crest(mocker):
    ...
    mocker.patch(
        ...
        return_value=MagicMock(
            spec=pygame.Surface,
            convert_alpha=lambda: MagicMock(spec=pygame.Surface, get_size=lambda: (100, 50)),
        ),
    )
    mocker.patch(
        "pygame.Surface",
        return_value=MagicMock(
            spec=pygame.Surface, get_at=lambda *args: (0, 0, 0, 255), get_size=lambda: (100, 50)
        ),
    )
    ...
    wave_layer = ParallaxLayer(
        "assets/sprites/wave1/waves1.png", z_depth=2, vertical_percent=0.7, scroll_speed=0.5
    )
    ...
    boat_layer.draw(
        mock_screen, scroll_x=0, environment_y=400, environment_layer_for_tilt=wave_layer
    )
    ...
    boat_layer.draw(
        mock_screen, scroll_x=300, environment_y=400, environment_layer_for_tilt=wave_layer
    )
    ...
    boat_layer.draw(
        mock_screen, scroll_x=150, environment_y=400, environment_layer_for_tilt=wave_layer
    )
