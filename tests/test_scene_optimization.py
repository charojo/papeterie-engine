...
def test_optimize_scene_mocked(mock_image_open, mock_gemini_client):
    ...
    mock_image_response.candidates = [
        MagicMock(content=MagicMock(parts=[mock_image_part]))
    ]
    mock_image_response.usage_metadata = None

    # Side effect: First call is decompose (text), subsequent calls are extraction (image)
    # Note: decompose_scene uses model_name="gemini-2.5-flash", edit_image uses "gemini-3-pro-image-preview"
    ...
    # Loading prompts - simplified for test (mock reading files if strictly needed, but let's assume we pass strings)
    ...
    img_bytes = client.extract_element_image("fake/path/image.png", "Draw background", "System Instruction")
