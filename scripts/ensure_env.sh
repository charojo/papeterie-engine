...
# Pre-check: If VIRTUAL_ENV is set but doesn't point to our root .venv, unset it
# so uv doesn't complain during script execution.
if [ -n "$VIRTUAL_ENV" ]; then
    EXPECTED_VENV="$PROJECT_ROOT/.venv"
    if [ "$VIRTUAL_ENV" != "$EXPECTED_VENV" ]; then
        echo -e "${YELLOW}Warning: VIRTUAL_ENV mismatch ($VIRTUAL_ENV). Unsetting for this session...${NC}"
        unset VIRTUAL_ENV
    fi
fi
